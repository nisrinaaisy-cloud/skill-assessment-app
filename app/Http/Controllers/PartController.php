...
class PartController extends Controller
{
	...
	public function store(Request $request)
	{
		$request->validate([
			...
			'division_id' => 'nullable|exists:divisi,id',
			'is_active' => 'required|boolean',
			'sub_process' => 'nullable|array',
			'sub_process.*' => 'nullable|exists:sub_processes,id',
		], [
	...
	public function update(Request $request, string $id)
	{
		...
		$request->validate([
			...
			'nama_part.required' => 'Nama Part wajib diisi.',
			'division_id.exists' => 'Divisi yang dipilih tidak valid.',
			'sub_process.required' => 'Minimal pilih satu Sub Proses.',
			'sub_process.min' => 'Minimal pilih satu Sub Proses.',
		]);

		$duplicatePart = Part::where('no_part', $request->no_part)
			->where('id', '!=', $part->id)
			->first();
